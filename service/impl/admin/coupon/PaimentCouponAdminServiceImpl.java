...
import java.util.List;
@Service
public class PaimentCouponAdminServiceImpl implements PaimentCouponAdminService {
    ...
    public PaimentCoupon create(PaimentCoupon t) {
        Coupon coupon = couponService.findById(t.getCoupon().getId());
        if (coupon == null) {
            throw new EntityNotFoundException("Coupon not found with id: " + t.getCoupon().getId());
        }

        if (coupon.getNombreCollaboratorInscrit() < coupon.getNombreInscriptionMax()) {
            coupon.setNombreCollaboratorInscrit(coupon.getNombreCollaboratorInscrit() + 1);
            couponService.update(coupon);

            t.setCoupon(coupon);

            PaimentCoupon existingPaimentCoupon = findByReferenceEntity(t);
            PaimentCoupon saved;
            if (existingPaimentCoupon == null) {
                saved = dao.save(t);
            } else {
                saved = null;
            }
            ...
            throw new IllegalStateException("The number of registrations has reached the maximum limit for this coupon.");
        }
    }
    ...
    public void findOrSaveAssociatedObject(PaimentCoupon t){
        if( t != null) {
            ...
            t.setPaimentCouponState(paimentCouponStateService.findOrSave(t.getPaimentCouponState()));
        }
    }
    ...
    @Autowired
    private PaimentCouponStateAdminService paimentCouponStateService ;

    public PaimentCouponAdminServiceImpl(PaimentCouponDao dao) {
        this.dao = dao;
    }

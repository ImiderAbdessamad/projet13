...
@RestController
@RequestMapping("/api/collaborator/country/")
public class CountryRestCollaborator {


@Autowired
 CountryCollaboratorService countryCollaboratorService;
    ...
    public ResponseEntity<CountryDto> findByLibelle(@PathVariable String libelle) {
	    Country t = countryCollaboratorService.findByLibelle(libelle);
        if (t != null) {
            CountryDto dto = converter.toDto(t);
            return getDtoResponseEntity(dto);
        }
    ...
    @Autowired private CountryCollaboratorService service;
